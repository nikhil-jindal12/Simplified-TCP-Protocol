...
def server_main():
    ...
    print(str(time.time()), f"Server: Received data from client:\n{buf[0].decode()}")
    ...
    with open(file_name, "rb") as f:
        file_data = f.read()
        print(str(time.time()), f"Server: Sending file '{file_name}' to the client...")
        server_socket.send(file_data)

    # Send randomly generated data to the client
    random_data = generate_random_data(128)
    print(str(time.time()), f"Server: Sending randomly generated data to the client...")
